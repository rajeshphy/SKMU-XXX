...
$$ (\Box + m^2) \psi = 0 $$
-->
#### Question 2: Find the plane wave solution to the Klein-Gordon equation and verify it satisfies the equation.
...
<!--
**Solution:**
Start with the Klein-Gordon equation in momentum space:
$$ (E^2 - p^2 - m^2) \psi = 0 $$
To linearize, assume $$E$$ and $$\vec{p}$$ are linear operators:
$$ (i\hbar \frac{\partial}{\partial t})^2 = (i\gamma^0 \partial_0 + i\gamma^i \partial_i)^2 = (\gamma^\mu p_\mu)^2 $$
We want:
$$ \gamma^\mu p_\mu \gamma^\nu p_\nu = (p_\mu p^\mu) $$
Choose $$\gamma^\mu$$ matrices that satisfy the Clifford algebra:
$$ \{\gamma^\mu, \gamma^\nu\} = 2g^{\mu\nu} $$
Thus, we obtain the Dirac equation:
$$ (i\gamma^\mu \partial_\mu - m) \psi = 0 $$
-->
